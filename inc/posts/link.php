<article class="h-entry post <?php if (is_single()){ echo('single'); }?> link row">
  <div class="col-lg-3">
    <p class="date">
  ...
  <div class="col-lg-9">
    <?php if (!is_single()){ ?>
      ...
      <p class="my-0"><a href="<?php the_field('link_url'); ?>" class="muted"><?php echo str_replace('https://', '', get_field('link_url')); ?></a></p>
    <?php } else { ?>
      <h3 class="h3 mt-0"> <a href="<?php the_field('link_url'); ?>"><?php echo str_replace('https://', '', get_field('link_url')); ?></a></h3>
    <?php } ?>
    <div class="e-content">
      <?php the_content(); ?>
    </div>
